...
fit <- maxnet::maxnet(p, x, f = formula, regmult = 1)

coefficient <- data.frame(feature = names(fit$betas),
                          coefficient = unname(fit$betas))
utils::write.csv(coefficient, file.path(output_dir, "coefficients.csv"),
                 row.names = FALSE)
...
utils::write.csv(prediction, file.path(output_dir, "predictions.csv"),
                 row.names = FALSE)
